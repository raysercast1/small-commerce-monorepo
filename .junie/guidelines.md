...
- Dev server: `ng serve` (Angular build dev-server).
...
Commands:
- Run all tests (watch mode): `npm test` (vitest). Vitest watches by default unless mode is set to "ci" in vite config.
...
- Minimal demo spec (works with this setup):
  ...
  ```
- Run locally: `npm test` (watch) or `npx vitest --run`.
...
Notes and caveats
- The repo is configured for Vitest, not Karma; ignore legacy README references to Karma/ng test. Use `npm test` (Vitest) instead.
...
- SSR: Build generates server bundle; use `npm run serve:ssr:myapp` to run locally. Ensure any browser-only APIs are guarded (e.g., isPlatformBrowser) to avoid SSR crashes.
...
- Dev: `npm start`
- Build: `npm run build`
- Unit tests: `npm test` (watch) | `npx vitest --run` | `npm run test:ui`
